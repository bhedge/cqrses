...
for (let x in dbTypes) {
  ...
  t.test('Db should write an event0 and default to version 0 if not provided', async function (t) {
    ...
    let updatedEvent0 = Object.assign({}, event0, { version: 0 })
    t.same(result, updatedEvent0, 'should return current state matching event')
    t.end()
  })
  ...
  t.test('Db should read event0 by id', async function (t) {
    ...
    let updatedEvent0 = Object.assign({}, event0, { version: 0 })
    t.same(result, updatedEvent0, 'should return event0')
    t.end()
  })
  ...
  t.test('Db should read 4 events by aggregateId', async function (t) {
    ...
    let updatedEvent0 = Object.assign({}, event0, { version: 0 })
    let updatedEvent3 = Object.assign({}, event3, { version: 3 })
    let compare = [updatedEvent0, event1, event2, updatedEvent3]
  ...
  t.test('Db should throw error on write the first event has version !== 0', async function (t) {
    ...
    t.rejects(db.mutate.write({ collection: 'eventSource', event: badEvent0 }), 'E_DB_WRITE_EVENT_VERSION_MISMATCH')
    t.end()
  })
  ...
  t.test('Db should throw an error if the broker fails to publish the event', async function (t) {
    ...
    let badBroker = {}
    badBroker.publish = async function () {
      throw Error('broker error.')
    }

    let result = await db.mutate.write({ collection: 'eventSource', event: emitEvent, pubBroker: badBroker })
  ...
  t.test('Db worked with a passed in DB connection', async function (t) {
    ...
    let result = await db.mutate.write({ collection: 'eventSource', event: testEvent, dbWriter: newDb })

    t.same(result, testEvent, 'should return the test_event')
    t.end()
  })
}
